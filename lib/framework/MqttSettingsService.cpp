...
void MqttSettingsService::onMqttConnect(bool sessionPresent) {
  Log.trace(F("Connected to MQTT, "));
  if (sessionPresent) {
    Log.trace(F("with persistent session" CR));
  } else {
    Log.trace(F("without persistent session" CR));
  }
}

void MqttSettingsService::onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
  Log.trace(F("Disconnected from MQTT reason: %d" CR), (uint8_t)reason);
...
void MqttSettingsService::onStationModeGotIP(WiFiEvent_t event, WiFiEventInfo_t info) {
  if (_state.enabled) {
    Log.trace(F("WiFi connection dropped, starting MQTT client." CR));
    onConfigUpdated();
  }
}
...
void MqttSettingsService::onStationModeDisconnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  if (_state.enabled) {
    Log.trace(F("WiFi connection dropped, stopping MQTT client." CR));
    onConfigUpdated();
  }
}
#elif defined(ESP8266)
...
void MqttSettingsService::configureMqtt() {
  ...
  if (_state.enabled && WiFi.isConnected()) {
    Log.trace(F("Connecting to MQTT..." CR));
